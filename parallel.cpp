...
m64p_handle configRspParallel = NULL;
...
extern "C"
{
	...
	#ifdef CONFIG_GUI
		CFG_HLE_GFX = ConfigGetParamBool(configRspParallel, KEY_HLE_GFX);
		CFG_HLE_AUD = ConfigGetParamBool(configRspParallel, KEY_HLE_AUD);
	#else // CONFIG_GUI
									 ...
									 void (*DebugCallback)(void *, int, const char *))
	{
		...
		ConfigOpenSection("RSP-Parallel", &configRspParallel);
		ConfigSetDefaultBool(configRspParallel, KEY_HLE_GFX, 0, "Send display lists to the graphics plugin");
		ConfigSetDefaultBool(configRspParallel, KEY_HLE_AUD, 0, "Send audio lists to the audio plugin");
